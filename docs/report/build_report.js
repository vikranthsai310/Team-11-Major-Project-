...
add(
  ...
  P("The forecaster beats a moving-average baseline by 11.2 % in MAE but lags the series it predicts. Every policy, including the learned agent, produced **zero** capacity violations. On the validation split the constrained optimizer Pareto-dominates the tuned fixed-interval baseline, and the DQN agent reaches a per-user cost 9 % below any optimizer configuration while dominating two of them — yet an ablation shows it is queue-aware rather than congestion-aware. [[PENDING: one sentence with the test-split headline result after the final evaluation]]"),
  P("**Keywords:** Cardano, eUTXO, decentralized exchange, transaction batching, reinforcement learning, DQN, LightGBM, discrete-event simulation."),
);
...
add(
  ...
  P("Every reported result is produced in a simulator that replays real mainnet blocks. Its assumptions are stated in Section 5.9 and their effect on validity is discussed in Section 8.12."),
...
add(
  ...
  ...Tbl("Software and libraries, with the reason for each choice", ["Layer", "Choice", "Why this and not the alternative"], [
    ...
    ["Tx building / on-chain (optional)", "PyCardano, Aiken", "Pure-Python transaction building; modern Cardano contract language"],
  ], [1800, 2800, 4400]),
...
add(
  ...
  ...Code([
    ...
    "  build/     estimator.py (size, execution units, fee, Gate A, Gate B)",
  ...
  P("[[Section 6.11 on on-chain validators applies only if the optional Phase 7 live demonstration is completed.]]"),
);
...
add(
  ...
  ...Tbl("Selected unit and property tests", ["ID", "Test", "Expected", "Result"], [
    ...
    ["T-L5", "Checkpoint reproducibility", "Reloaded checkpoint reproduces metrics", "[[PENDING]]"],
  ], [1000, 3400, 3200, 1400]),
  ...
  H2("7.5 Defects Found by Testing"),
  ...
  H2("7.6 Regression Guards for Corrected Assumptions"),
  ...Tbl("Permanent guards for each architecture decision", ["Decision", "Guard", "Status"], [
    ...
    ["ADR-006 pass-through fee", "T-O5", "Optional Phase 7"],
...
add(
  ...
  ...Callout("Status of this chapter", [
    "Sections 8.1–8.4 and the ablations are final. Sections 8.5–8.9 currently report **validation-split** results from Phases 4 and 5. The single test-split evaluation (8 policies × 3 arrival rates × 100 paired episodes, P3 over five seeds) is running; its main table with confidence intervals, F5, F6, F8 and the S1–S6 verdicts replace the marked placeholders when it completes. [[PENDING: final evaluation]]",
  ]),
  ...
  P("[[PENDING: main test-split table — L-mean, L-p95, C-user, X-rate, TP, F-jain for NULL, E1 (M=16), E2 (T=20), E3, P2 (D=120, N=1 and N=4), ORACLE and P3, each cell a median with 95 % bootstrap CI, significant improvements over the best static baseline marked.]]"),
  P("On the validation split (matched arrival rate, ten paired episodes), sweeping P2’s minimum batch size traces a frontier:"),
  ...
  P("[[PENDING: Figure F5 — L-p95 against C-user with 95 % CIs on both axes, from the test split.]]"),
  ...
  P("P3 dominates `p2(N=12)` and `p2(N=20)` on both axes and is dominated by nothing, reaching a per-user cost 9 % below any P2 configuration. It thus extends the achievable frontier, at a tenth of the specified training budget. Every seed produced zero Gate A violations and zero expired orders, and none collapsed. Seed variance is real: seed 0 behaves like a different policy (L-p95 123.5, mean batch 8.9 against about 13 for the others)."),
  H2("8.7 Latency Distributions"),
  P("[[PENDING: Figure F6 — latency CDF with the 0.95 crossing of each policy marked.]]"),
  ...
  ...Callout("Finding", [
    "**The agent is queue-aware, not congestion-aware.** This is the third independent route to the ADR-008 conclusion, after the F4 lag diagnostic and the ORACLE gap. The retrained ablation A3 is still owed; the inference-time version reported here points the same way.",
  ]),
  H2("8.9 Robustness Across Arrival Rates"),
  P("[[PENDING: Figure F8 — L-p95 by policy in light / matched / heavy panels on one shared scale.]]"),
  H2("8.10 Ablations"),
  ...
  P("A3 (inference-time) is reported in Section 8.8. [[PENDING: retrained A3 without fill_hat, and A4 with a linear latency penalty.]]"),
  ...
  ...Tbl("Success metrics S1–S6", ["ID", "Criterion", "Measured", "Verdict"], [
    ...
    ["S2", "Zero Gate A violations", "0 on validation; [[test]]", "[[PENDING]]"],
    ["S3", "L-p95 below best static baseline, significant", "[[test]]", "[[PENDING]]"],
    ["S4", "Expiry not worse than baseline", "[[test]]", "[[PENDING]]"],
    ["S5", "Pareto dominance on (L-p95, C-user)", "Validation: p2(N=4) dominates e2(T=20)", "[[PENDING]]"],
    ["S6", "Fairness not worse than baseline", "[[test]]", "[[PENDING]]"],
  ], [700, 3300, 3300, 1700]),
  H2("8.12 Threats to Validity"),
  ...Tbl("Threats to validity and their mitigation", ["Threat", "Mitigation and residual risk"], [
    ...
    ["Code provenance of manifests", "Earlier experiments ran from uncommitted code, so manifests record the parent revision; data and parameters are pinned by checksum and configuration hash. The test evaluation carries the same gap and is stated rather than re-run, since re-running would spend the test split twice"],
...
add(
  ...
  ...Tbl("Goals G1–G6 against the evidence", ["Goal", "Status", "Evidence"], [
    ["G1 Reduce mean and p95 latency vs static batchers", "[[PENDING test split]]", "Validation: p2(N=4) beats tuned E2 on L-p95 at lower cost; greedy remains the latency extreme"],
    ["G2 Reduce per-user cost by amortization", "Achieved on validation", "P3 reaches a C-user 9 % below any P2 configuration while dominating two of them"],
    ["G3 Never violate capacity limits", "Achieved, with a stated condition", "Zero Gate A violations in every run; exact only under perfect estimation (A5)"],
    ...
    ["G5 Reproducible evaluation", "Partially achieved", "Seeds, checksums and configuration hashes pinned; earlier manifests record the parent code revision"],
  ...
  B("**Under-trained agent** — P3 was trained at a tenth of the specified budget."),
