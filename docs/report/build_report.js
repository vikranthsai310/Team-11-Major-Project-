...
add(
  ...
  ...Tbl("On-chain and live-mode tests", ["ID", "Test", "Result"], [
    ...
    ["T-O6", "One end-to-end swap settles on preprod", "[[PENDING: requires deployment]]"],
  ], [900, 6100, 2000]),
...
add(
  ...
  ...Tbl("Phase 7 exit criteria", ["Criterion", "Status"], [
    ...
    ["T-O6 — one swap settles on preprod, transaction hash recorded", "[[PENDING]]"],
    ["Estimator within ±5 % on size and ±10 % on execution units against D4", "[[PENDING]]"],
    ["`07-CONSTRAINTS-COST-MODEL.md` §4 per-order costs updated from measurement", "[[PENDING]]"],
  ], [6000, 3000]),
  P("Two outcomes of the calibration can be anticipated. The fee replay (V4) already found the fee formula under-estimating script-bearing transactions by a median of 29 %, and a DEX batch is script-bearing. And ADR-006 recorded a cost of its own design: computing `fee / n` on chain consumes execution units, which slightly lowers the Gate A cap. Both will be measured rather than assumed, and the per-order estimates used throughout the simulator revised if they fall outside tolerance."),
...
add(
  ...
  B("Deploy to preprod, settle the end-to-end swap, and calibrate the estimator from D4, folding in the reference-script fee term."),
