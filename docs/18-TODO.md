...
| P7-6 deploy | ✅ **DEPLOYED to preprod 2026-09-15** — tx `555ee19bd06d2151931035039dac883b560d9f93bb35b3fd1efa24a30f767372`, block 5,178,709, slot 133,767,575, fee 181,913 lovelace. Pool verified on chain: 100 tADA + 1,000,000 TEAM11 + 1 POOL NFT, inline datum equal to the recorded deployment. Record `onchain/deployment.preprod.json`. **Defect found on first live read:** Blockfrost returns inline datums as `RawCBOR`, which `read_orders` treated as undecodable — every real order would have been silently skipped. Fixed with `datum_bytes`, regression-tested. Built from —  `scripts/deploy_dex.py` (dry run by default) derives the deployment from the batcher key and a mint window, mints `TEAM11` + one `POOL` NFT under a key-**and**-time-locked policy (no second NFT can ever be minted), opens the pool. Record holds hashes only and is re-verified on load. Dry run verified with real `aiken blueprint apply` |
...
| P7-8 live daemon | ✅ **ran live on preprod 2026-09-15** — shadow run (3 blocks, WAIT on an empty queue), then live: one SUBMIT, one LOCKED block, `included`, clean stop with the pool free. Built as — `scripts/run_batcher.py` over `live/daemon.py`: one decision per new block, none while a batch holds the pool, the simulator's own `_enforce` (identity-tested), FIFO by on-chain arrival slot, E4 forecast over real recent blocks. **Shadow mode by default** (decides and builds the signed batch, submits nothing). Handles inclusion, TTL expiry, submission rejection, and a lagging API view of its own batch (`stale_view`). Ctrl+C stops only once the pool is free and starts no new batch. D4 rows to `data/live/` (gitignored). 16 offline tests |
| P7-11 orders | ◐ `scripts/place_order.py` (dry run by default), `build_cancel_tx` for T-O1 on chain |
| P7-11 end-to-end swap | ✅ **T-O6 PASS 2026-09-15** — order `6ffd8101…25ac41` (sell 10 tADA, min 85,000 TEAM11); live P2 batch `b5201c0a…c73af1` submitted slot 133,785,075, included 133,785,125 (block 5,179,501), fee 322,697. Verified on chain: user got 90,661 TEAM11 + 3.679758 tADA (tagged payout), pool 110 tADA / 909,339 TEAM11 + NFT, no orders left. Record `experiments/phase7-preprod-demo/`. **Second live defect:** users charged for the estimator's fee (406,931) above the real one (320,242) → `order.ak` rejected during evaluation, nothing submitted; fixed with a two-pass build (`df58756`) and a strict fake chain; daemon now logs `build_failed` |
| P7-12 dashboard | ✅ `python scripts/dashboard.py` → `http://127.0.0.1:8050`, localhost-only (refuses any other bind), standard library server, no CDN. **How it works** (explainer), **Run** (replays any of the 100 test days through `run_episode` for P2 / E1–E3 / ORACLE: congestion chart, queue with D_MAX meter, pool LOCKED/FREE, WAIT/SUBMIT with Gate A and Gate B shown separately, decision log, running tiles), **Results** (test-split table with CIs, Pareto and latency CDF, per traffic level — verified to reproduce the report’s 19 % / 43 % / 125 s vs 131 s), **Live on preprod** (pool reserves, waiting orders, recorded swap transactions with explorer links, via Blockfrost, 15 s cache). 13 tests; checked in the browser against real data |
| P7-9 calibration | ◐ first D4 row, n = 1: size est 800 / actual 2,682 (under 3.4×, unsafe — inline scripts not modelled); mem 2.5 M / 473,756 and steps 1.0 G / 166 M (over ~5–6×, safe); fee 406,931 / 322,697. **±5 % / ±10 % not met**; per-order costs need multi-order batches; reference scripts would close most of the size gap. `07` §4 not yet updated |
